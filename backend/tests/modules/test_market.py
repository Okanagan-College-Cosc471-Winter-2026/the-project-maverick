...
def test_list_stocks(client: TestClient, market_data):  # noqa: ARG001
...
def test_list_stocks_only_active(client: TestClient, market_data):  # noqa: ARG001
    response = client.get("/api/v1/market/stocks")
    assert response.status_code == 200
    data = response.json()
    symbols = [s["symbol"] for s in data]
...
def test_get_stock(client: TestClient, market_data):  # noqa: ARG001
...
def test_get_stock_case_insensitive(client: TestClient, market_data):  # noqa: ARG001
    response = client.get("/api/v1/market/stocks/aapl")
    assert response.status_code == 200
    data = response.json()
    assert data["symbol"] == "AAPL"
...
def test_get_stock_not_found(client: TestClient, market_data):  # noqa: ARG001
    response = client.get("/api/v1/market/stocks/FAKE")
    assert response.status_code == 404


def test_get_ohlc(client: TestClient, market_data):  # noqa: ARG001
...
def test_get_ohlc_days_param(client: TestClient, market_data):  # noqa: ARG001
...
def test_get_ohlc_not_found_symbol(client: TestClient, market_data):  # noqa: ARG001
