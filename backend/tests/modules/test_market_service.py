...
from app.modules.market.schemas import StockRead, OHLCRead

def test_list_stocks_returns_stock_read_objects(db: Session, market_data):
...
def test_get_ohlc_converts_timestamps(db: Session, market_data):
...
def test_get_ohlc_empty_range(db: Session, market_data):
    ...
    ohlc_data = MarketService.get_ohlc(db, "GOOGL", days=30) # GOOGL exists but has no prices in seed
