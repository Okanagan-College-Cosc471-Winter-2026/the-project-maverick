...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-csv", default=None, help="CSV source for ml.market_data_15m-compatible data (alternative to parquet).")
...
def resolve_source_csv_path(args: argparse.Namespace) -> Path | None:
    if not getattr(args, "source_csv", None):
        return None
    return resolve_runtime_path(args.source_csv)


def load_csv_source(csv_path: Path) -> pd.DataFrame:
    if not csv_path.exists():
        raise RuntimeError(f"CSV source does not exist: {csv_path}")
    LOGGER.info("Reading CSV source %s ...", csv_path)
    bars = pd.read_csv(csv_path, low_memory=False)
    return normalize_loaded_bars(bars, source_name=str(csv_path))


def describe_source(args: argparse.Namespace) -> str:
    csv_path = resolve_source_csv_path(args)
...
def open_source(args: argparse.Namespace):
    csv_path = resolve_source_csv_path(args)
    if csv_path is not None:
        LOGGER.info("Using CSV source %s", csv_path)
...
WHERE window_ts::date BETWEEN :start_date AND :end_date
  AND (:symbols::text IS NULL OR symbol = ANY(string_to_array(:symbols, ',')))
"""
...
def infer_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    load_start = (as_of_date - pd.Timedelta(days=10)).strftime("%Y-%m-%d")
    ...
    origin_bar_ts = latest_available_origin_bar_ts(dataset, as_of_date)
    as_of_rows = dataset[dataset["bar_ts"] == origin_bar_ts].copy()
    if as_of_rows.empty:
        raise RuntimeError(f"No origin rows were found for inference on or before {as_of_date.date()}.")
...
def latest_available_origin_bar_ts(dataset: pd.DataFrame, requested_as_of: pd.Timestamp) -> pd.Timestamp:
    eligible = dataset.loc[dataset["trade_date"] <= requested_as_of, "bar_ts"].dropna()
    if eligible.empty:
